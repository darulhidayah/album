...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [orgRes, progRes, allMediaCounts] = await Promise.all([
      ...
      supabase.from("media").select("program_id").eq("organization_id", ORG_ID),
    ]);

    const progCountMap: Record<string, number> = {};
    (allMediaCounts.data || []).forEach((r: any) => {
      if (r.program_id) {
    ...
    return NextResponse.json({
      ...
      total_media: (allMediaCounts.data || []).length,
      data: mediaList || [],
      pagination: {
        page,
        limit,
